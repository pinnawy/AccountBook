...
namespace AccountBook.Silverlight.Web
{
    ...
    /// </summary>
    [EnableClientAccess]
    public class UserService : DomainService, IUserBLL
    {
            ...
            string password)
        {
            ...
            if (!Roles.RoleExists(UserService.DefaultRole))
            {
                Roles.CreateRole(UserService.DefaultRole);
            }
            ...
            {
                return UserService.ConvertStatus(createStatus);
            }

            // Assign the user to the default role.
            // This will fail if role management is disabled.
            Roles.AddUserToRole(user.UserName, UserService.DefaultRole);
        ...
        {
            return _userBLL.GetUser(username);
        }

        public List<UserInfo> GetUserList()
        {
            return _userBLL.GetUserList();
        }
    ...
    /// <summary>
    /// An enumeration of the values that can be returned from <see cref="UserService.CreateUser"/>
    /// </summary>
